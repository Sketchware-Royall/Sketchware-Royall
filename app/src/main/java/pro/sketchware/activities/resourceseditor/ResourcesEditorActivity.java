...
public class ResourcesEditorActivity extends BaseAppCompatActivity {
	...
	public String integersFilePath;
	public IntsEditor intsEditor;
	public yq yq;
	...
	public boolean onOptionsItemSelected(@NonNull android.view.MenuItem item) {
		...
		if (id == R.id.action_save) {
			...
			switch (currentItem) {
				...
				case 5 -> {
					intsEditor.hasUnsavedChanges = true;
	...
	private void setupViewPager() {
		...
		new TabLayoutMediator(binding.tabLayout, binding.viewPager, (tab, position) -> {
			switch (position) {
				case 0 -> tab.setText("strings" + variant + ".xml");
				case 1 -> tab.setText("colors" + variant + ".xml");
				case 2 -> tab.setText("styles" + variant + ".xml");
